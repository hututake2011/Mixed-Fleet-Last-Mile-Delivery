# This file select input nodes from orginal CSTDM nodes
...
dataframe = pd.read_csv('CSTDM_nodes_Original.csv')

LargeNodeSet = [4191, 3868, 3704, 3864, 3712, 3895, 3940, 3915, 3784, 3983, 3952, 4944, 4989, 4917, 4975, 4906, 4959,
...
LargeSet = dataframe.loc[dataframe['NID'].isin(LargeNodeSet)]
LargeSet.to_csv('LargeNodeSet.csv', index=False)

SmallNodeSet = [4191, 4943, 4991, 4975, 4181, 4161, 5046, 5068, 4105, 3947, 3952, 4036, 4119,
                4254, 4057, 4001, 3979, 4447, 4313]

SmallSet = dataframe.loc[dataframe['NID'].isin(SmallNodeSet)]
SmallSet.to_csv('SmallNodeSet.csv', index=False)
...
smallAreaHighDemand = Gen_demand(para.HighDemand, len(SmallNodeSet))
largeAreaHighDemand = Gen_demand(para.HighDemand, len(LargeNodeSet))
largeArea2kDemand = Gen_demand(para.Demand2K, len(LargeNodeSet))
largeArea1kDemand = Gen_demand(para.Demand1K, len(LargeNodeSet))
...
def Gen_distance_time_Matrix(n, dataFrame, demand_node):
    ...
    for i in range(len(n)):
        for j in range(len(n)):
            if i != j:
                x1 = dataFrame.loc[dataframe['NID'] == n[i], 'X'].iloc[0]
                y1 = dataFrame.loc[dataframe['NID'] == n[i], 'Y'].iloc[0]
                x2 = dataFrame.loc[dataframe['NID'] == n[j], 'X'].iloc[0]
                y2 = dataFrame.loc[dataframe['NID'] == n[j], 'Y'].iloc[0]
...
largeMatrix, largeTimeMatrix = Gen_distance_time_Matrix(LargeNodeSet, LargeSet, largeAreaHighDemand)
